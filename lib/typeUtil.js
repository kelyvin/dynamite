...
function valueToObject(value) {
  ...
  default:
    if (Array.isArray(value)) {
      ...
      if(value.length > 0 && value[0].hasOwnProperty('arrayType') && value[0].arrayType === 'set'){
...
function getAttributeAlias(key) {
  ...
  if (!isAlphaNumeric(key)) {
    // If the key already has a placeholder hash key, then return it as is and don't convert anything
    if (getExpressionAttributePlaceholder(key)) return key
...
/**
 * Converts attribute values into a format that is parsable
 * @see http://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ExpressionPlaceholders.html#ExpressionAttributeNames
 * @return {string} The alias. May just be the key itself if an alias is not needed.
 */
function getAttributeValue(key) {
  // Parses keys that have a hash to indicate updating a map
  if (!isAlphaNumeric(key)) {
    return new Buffer(key).toString('hex')
  }
...
function getExpressionAttributePlaceholder(key) {
 // Allows placeholder keys to support map updates
 var keysWithin = key.split('.')

 if (keysWithin.length > 1) {
   for (var i = 0; i < keysWithin.length; i++) {
     if (keysWithin[i].startsWith('#')) {
       return keysWithin[i]
     }
   }
 }
...
function buildAttributeNames(attrList) {
  ...
  attrList.map(function (key) {
    if (needsAttributeAlias(key)) {
      var placeholderKey = getExpressionAttributePlaceholder(key)

      // If a placeholder key is defined, simply use that key
      if (placeholderKey) {
        result[placeholderKey] = placeholderKey.slice(1)
      } else {
